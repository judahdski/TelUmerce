...
class PaymentInfo {
  ...
  factory PaymentInfo.fromJson(Map<String, dynamic> json) => PaymentInfo(
        opsiKirim: List<OpsiKirim>.from(
            json["opsiKirim"].map((x) => OpsiKirim.fromJson(x))),
        user: json["user"][0],
        metodePembayaran: List<MetodePembayaran>.from(
            json["metode_pembayaran"].map((x) => MetodePembayaran.fromJson(x))),
      );
  ...
  Map<String, dynamic> toJson() => {
        ...
        "metode_pembayaran":
            List<dynamic>.from(metodePembayaran.map((x) => x.toJson())),
      };
}
